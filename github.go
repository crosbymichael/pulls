...
func getMaintainerManagersIds(pth string) (*[]string, []*Maintainer, error) {
	...
	for scanner.Scan() {
		m, err := parseMaintainer(scanner.Text())
		if err != nil {
			fmt.Println(err, pth)
			continue
		}
...
func createMaintainerManagersDirectoriesMap(pth, cpth, maintainerEmail, userName string) error {
	...
	for _, name := range names {
		if strings.EqualFold(name.Name(), MaintainerManagersFileName) {
			...
			var ids = &[]string{}
			ids, idsFound, err = getMaintainerManagersIds(path.Join(pth, name.Name()))
	...
	maintainersIdsCurrentDirectory := append([]*Maintainer{}, (idsFound)...)
	maintainersDirMap[tmpcpth] = maintainersIdsCurrentDirectory
