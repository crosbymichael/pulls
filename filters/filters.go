...
func GetIssueFilter(c *cli.Context) IssuesFilter {
	...
	numVotes := c.Int("greater")
	if c.Bool("votes") || numVotes > 0 {
		FILTER_VOTES = numVotes
		filter = combineIssues(filter, voteIssuesFilter)
	}
...
func voteIssuesFilter(issues []*gh.Issue, err error) ([]*gh.Issue, error) {
	if err != nil {
		return nil, err
	}
	...
	for _, issue := range issues {
		...
		if issue.Comments >= FILTER_VOTES {
